...
Similar to analysis.py for patents, but prompts are tuned for
academic research context — focus on methodology, university clusters,
emerging research directions, and patent-thesis proximity signals
(research that is likely to become IP within 2-5 years).
"""
...
async def analyze_thesis_batch(
    ...
    # Reuse the LLM routing from analysis.py
    ...
    with get_session() as session:
        session.add(result)
        session.flush()
        log.info("thesis analysis: stored result id=%d", result.id)
