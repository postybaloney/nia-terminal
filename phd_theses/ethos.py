...
Docs: https://ethos.bl.uk/
API:  https://ethos.bl.uk/api/

EThOS is the UK's national thesis service run by the British Library.
~600k theses from all UK universities, dating back to the 1700s.

Free to search. Full text availability varies by thesis.
API key optional — register at https://ethos.bl.uk/Register.do for
higher rate limits, but basic search works without one.
"""
...
class EThOSIngestor(BaseThesisIngestor):
    ...
    async def _fetch_query(
        ...
        for raw in data.get("data", {}).get("records", []) or []:
            try:
    ...
    def _normalize(self, raw: dict, query: str) -> NormalizedThesis | None:
        ...
        # EThOS returns author as {firstName, lastName}
