...
# A "⚠ OPEN" blockquote is a note from the author to the author: a decision not
# yet made, usually one for a lawyer. Unlike the <!-- --> blocks these are
# written as markdown blockquotes, so without this they RENDER — and the first
# published draft of the privacy notice would have carried, in a visible
# callout on a public legal page:
#
#     "⚠ OPEN — for your lawyer. If GDPR Article 3(2) applies to this site,
#      you may owe a written EU representative under Article 27 ... This is
#      the exact and only ground on which LocateFamily.com was fined €525,000."
#
# That is an admission of a known, unresolved compliance gap, published by the
# controller, on the page a regulator would read first. They are stripped from
# the output and reported to the console instead, so the author is reminded on
# every build without the reminder reaching the reader.
_OPEN_NOTE = re.compile(r"(?:^> *⚠ *\*\*OPEN.*?(?:\n(?!>).*?)??)(?=\n{2,}|\Z)",
                        re.S | re.M)
...
def strip_open_notes(md: str, source: str) -> str:
    """Remove ⚠ OPEN author notes from the output, and say what was removed."""
    notes = []

    def _drop(m):
        first = m.group(0).lstrip("> ").replace("\n> ", " ").replace("\n", " ")
        notes.append(" ".join(first.split())[:110])
        return ""

    out = _OPEN_NOTE.sub(_drop, md)
    for n in notes:
        print(f"     ⚠ unresolved, withheld from the page: {n}…")
    if "⚠" in out and "OPEN" in out:
        raise SystemExit(
            f"\n  REFUSING TO BUILD {source}\n\n"
            f"  An ⚠ OPEN note survived stripping. It would render as a visible\n"
            f"  callout on a public legal page. Fix the note's formatting (it\n"
            f"  must be a '> ⚠ **OPEN' blockquote) or resolve it.\n")
    return out


def resolve(md: str, subs: dict, source: str) -> str:
    md = _COMMENT.sub("", md)
    md = strip_open_notes(md, source)
...
{body}
  <footer>
    ...
    lives in <code>legal/</code> and is the version of record.
  </footer>
</div>
</body></html>"""
